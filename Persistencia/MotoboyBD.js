...
export default class MotoboyBD{
    ...
    async incluir(motoboy){
        if(motoboy instanceof Motoboy){
            ...
            const sql = "INSERT INTO newmotoboys(nome, endereco, cpf, telefone, dataCadastro, codPedido, observacao) VALUES(?,?,?,?,?,?,?)";
    ...
    async alterar(motoboy){
        if(motoboy instanceof Motoboy){
            const conexao = await Conectar();
            const sql = "UPDATE newmotoboys SET nome=?, endereco=?, cpf=?, telefone=?, dataCadastro=?, codPedido=?, observacao=? WHERE codigo=?";
            const valores = [motoboy.nome, 
                             motoboy.endereco,
                             motoboy.cpf, 
    ...
    async consultar(termo){
        ...
        for (const row of rows){
            ...
            const motoboy = new Motoboy(row['codigo'], row['nome'], row['endereco'], row['cpf'], row['telefone'], row['dataCadastro'], pedido, row['observacao']);
